...
class SnlSpider(scrapy.Spider):
    ...
    def parseRatingsEpisode(self, response):
        ...
        try:
            ...
            logging.warn(
                'Insufficient user ratings for episode at {}'.format(response.url))
            raise StopIteration
    ...
    def parseEpisode(self, response, target_tid=None):
        ...
        for epInfoTr in response.css("table.epGuests tr"):
            ...
            # e.g. ("<td><p>Host:</p></td>", "<td><p>Anna Faris</p></td>")
            fieldTd, valueTd = epInfoTd
    ...
    def parseSketchDiv(self, sketchInfo, order, epid, extra_cast):
        ...
        if title_url:
            if title_url.startswith('/Sketches/'):
                ...
                logging.warn(
                    'Unrecognized title url format: {}'.format(title_url))

        aids_this_title = {}
        # Parse the Appearances in this title
        for cast_entry in sketchInfo.css(".roleTable > tr"):
            try:
                actor, app = self.parse_cast_entry_tr(
                    cast_entry, extra_cast, title['tid'])
    ...
    def parse_cast_entry_tr(self, row, extra_cast_lookup, tid):
        ...
        if actor_name is None:
            raise EmptyCastRowException(
                'Found no name for cast tr in sketch with tid={}'.format(tid))
        ...
        if actor_name == 'Jack Handey':
            ...
            if not actor_class:
                logging.warn(
                    "No class found in actor cell {}".format(actor_cell))
                capacity = 'unknown'
            else:
    ...
    def parse_role_cell(self, role_td, appearance, tid):
        ...
        if role_link:
            ...
            if href.startswith('/Impressions/'):
                ...
                raise Exception(
                    'Unrecognized role URL in sketch with tid={}: {}'.format(tid, href))
        return appearance
