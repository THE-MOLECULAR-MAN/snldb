...
if SCRAPE_FULL_EP:
    ...
    full_episode_tids = [full_episode['epid'] +
                         str(i+1) for i in range(full_episode['ntitles'])]

    def dedupe_list(lst): return list(set(lst))
...
def test_lovers(basket):
    ...
    actor_names = {'Winona Ryder', 'Rachel Dratch',
    ...
    assert actors['Rachel Dratch'] == Actor(
        aid='Rachel Dratch', type='cast', url='/Cast/?RaDr')
    ...
    assert_item_props(wino, capacity='host', role='Clarissa',
                      impid=None, charid=None, voice=False)
    rd = apps['Rachel Dratch']
    assert_item_props(rd, capacity='cast',
                      role='Virginia Klarvin', impid=None, charid=559)
...
def test_cameo(basket):
    ...
    assert_item_props(alec, capacity='cameo', role=None,
                      impid=None, charid=None, voice=False)
