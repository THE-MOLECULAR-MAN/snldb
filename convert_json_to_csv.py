...
def add_indices(tables):
    ...
    table_to_index = dict(episodes='epid', impressions='impid',
                          seasons='sid', titles='tid', actors='aid')
...
def add_merge_cols(tables):
    ...
    mg = apps.merge(tables['titles'], on='tid').drop(
        ['category', 'order', 'skid', 'name'], axis=1)
    mg = mg.merge(tables['episodes'], on='epid').drop(
        ['aired', 'epno'], axis=1)
...
weekend_update_categories = {'Weekend Update',
                             'Saturday Night News', 'SNL Newsbreak'}
live_sketch_categories = {'Sketch', 'Musical Sketch',
                          'Show', 'Game Show', 'Award Show', 'Musical Performance'}
